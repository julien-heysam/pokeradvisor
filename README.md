...
3. Use the `/analyze` endpoint to upload a poker table image and get your win probability.

## Notes

- The card recognition is currently a stub. Replace `extract_poker_state` in `card_recognition.py` with real image processing logic.
- Odds calculation uses Monte Carlo simulation with the `treys` library.
...
4. Configure:
   ...
   - Start Command: `gunicorn -c gunicorn_config.py`
...
4. Set the start command: `gunicorn -c gunicorn_config.py`
